...
export default function BrandManager() {
  ...
  const countForBrand = (brand) =>
    products.filter(p => {
      const bname = (p.brand || p.name || '').toLowerCase();
      return bname.includes(brand.name.toLowerCase()) || bname.includes(brand.slug.toLowerCase());
    }).length;
  ...
  const handleEdit = (brand) => {
    setEditModal({ id: brand.id, name: brand.name, slug: brand.slug, description: brand.description || '' });
  ...
  // ── Table helpers ─────────────────────────────────────────────────────────
  const filtered = brands.filter(t =>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
            <Bookmark className="w-5 h-5 text-brand-600" /> Product Tags
          </h2>
          <p className="text-[11px] text-slate-400 mt-0.5">
            Manage brands for your product catalog. Link products to their manufacturer or vendor.
          </p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-12 text-center text-slate-400 text-xs">No brands found.</td>
                  </tr>
                ) : (
                  filtered.map((brand) => {
                    ...
                    return (
                      <tr key={brand.id} className={`group transition-colors ${isChecked ? 'bg-brand-50/40' : 'hover:bg-slate-50/60'}`}>
                        <td className="px-4 py-3">
                          <input type="checkbox" checked={isChecked} onChange={() => toggleSelect(brand.id)}
                            className="rounded border-slate-300 accent-brand-600" />
                        </td>
                        ...
                        <td className="px-4 py-3 min-w-[130px]">
                          ...
                          <div className="flex items-center gap-2 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleEdit(brand)} className="text-[10px] text-brand-600 font-semibold hover:underline">Edit</button>
                            <span className="text-slate-300">|</span>
                            <button onClick={() => handleDelete(brand.id, brand.name)} className="text-[10px] text-rose-500 font-semibold hover:underline">Delete</button>
                          </div>
                        </td>

                        <td className="px-4 py-3 text-slate-500 max-w-[180px] truncate" title={brand.description}>
                          {brand.description || <span className="text-slate-300">—</span>}
                        </td>

                        <td className="px-4 py-3 font-mono text-slate-500 text-[10px]">{brand.slug}</td>
      ...
      {editModal && (
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 overflow-hidden"
            style={{ animation: 'brandModalIn 0.18s ease' }}
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/60">
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                <Pencil className="w-4 h-4 text-brand-600" /> Edit Brand
              </h3>
