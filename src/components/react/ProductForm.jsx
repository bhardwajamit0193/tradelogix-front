...
export default function ProductForm({ productId }) {
  ...
  // Load brands from database to populate Brand spec values and avoid typos
  useEffect(() => {
    async function loadBrandsFromDb() {
      try {
        const res = await fetch(`${API_URL}/api/all-brands`);
        if (res.ok) {
        ...
        console.warn('Failed to load DB brands in ProductForm:', err);
      }
    }
    loadBrandsFromDb();
  }, []);

  const PREDEFINED_SPEC_KEYS = [
    'Brand',
    'Processor',
    'RAM',
    'Storage',
  ...
  const getSpecValueSuggestions = (keyName) => {
    if (!keyName) return [];
    const k = keyName.toLowerCase().trim();
    if (k === 'brand') {
      const base = ['HP', 'Dell', 'Lenovo', 'Apple', 'Asus', 'Acer', 'Samsung'];
      dbBrands.forEach((b) => {
        if (b && b.name && !base.some(x => x.toLowerCase() === b.name.toLowerCase())) {
          base.push(b.name);
        }
      });
      return base;
    ...
    if (k === 'ram' || k === 'memory') {
      return ['4 GB', '8 GB', '16 GB', '32 GB', '64 GB', '128 GB'];
    }
    if (k === 'storage' || k === 'ssd' || k === 'hdd' || k === 'rom') {
      return ['128 GB SSD', '256 GB SSD', '512 GB SSD', '1 TB SSD', '2 TB SSD', '1 TB HDD', '2 TB HDD'];
    }
    if (k === 'processor' || k === 'cpu') {
      return [
    ...
    if (k === 'display' || k === 'screen') {
      return ['13.3 Inches', '14.0 Inches', '15.6 Inches', '16.0 Inches', '17.3 Inches', '24 Inches', '27 Inches', '32 Inches'];
    }
    if (k === 'graphics' || k === 'gpu') {
      return ['Intel UHD Graphics', 'Intel Iris Xe', 'NVIDIA GeForce RTX 3050', 'NVIDIA GeForce RTX 4060', 'AMD Radeon Graphics'];
    }
    if (k === 'operating system' || k === 'os') {
      return ['Windows 11 Home', 'Windows 11 Pro', 'Windows 10 Pro', 'macOS', 'Ubuntu Linux', 'DOS'];
    }
    if (k === 'warranty') {
      return ['1 Year Onsite Warranty', '2 Years Warranty', '3 Years Commercial Warranty', '1 Year Carry-in'];
    }
    return [];
  };

  const handleQuickAddSpec = (keyName) => {
    const existingIdx = specifications.findIndex(
      s => s.key && s.key.toLowerCase().trim() === keyName.toLowerCase().trim()
    );
    if (existingIdx >= 0) {
      toast.info(`"${keyName}" specification is already added (Row #${existingIdx + 1})`);
      return;
    }
    setSpecifications(prev => [...prev, { key: keyName, value: '' }]);
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="border border-slate-200 bg-white rounded-2xl p-6 shadow-sm space-y-4">
                ...
                {/* Quick Add Predefined Specification Keys Bar */}
                <div className="flex flex-wrap items-center gap-1.5 p-3 bg-slate-50/80 rounded-xl border border-slate-200/80">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mr-1 flex items-center gap-1">
                    <Sparkles className="w-3 h-3 text-brand-600" /> Quick Add Specs:
                  </span>
                  {PREDEFINED_SPEC_KEYS.map((k) => (
                    <button
                      key={k}
                      type="button"
                      onClick={() => handleQuickAddSpec(k)}
                      className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-white hover:bg-brand-50 hover:text-brand-700 hover:border-brand-300 border border-slate-200 text-slate-700 transition-all shadow-2xs flex items-center gap-1 cursor-pointer"
                    >
                      <Plus className="w-3 h-3 text-brand-600" /> {k}
                    </button>
                  ))}
                </div>
                ...
                {specifications.length === 0 ? (
                  <div className="py-8 text-center rounded-xl border border-dashed border-slate-200 bg-slate-50/60 space-y-2">
                    ...
                    <p className="text-[11px] text-slate-400 max-w-sm mx-auto">
                      Click any of the quick-add buttons above or add custom specs to show formatted filters on the storefront.
                    </p>
                    <button
                      type="button"
                      onClick={handleAddSpecification}
                    ...
                    >
                      <Plus className="w-3.5 h-3.5 text-brand-600" /> Add Custom Specification
                    </button>
                  </div>
                ) : (
                  <div className="space-y-3">
                      ...
                      <table className="w-full text-left text-xs divide-y divide-slate-200">
                        ...
                        <tbody className="divide-y divide-slate-100 bg-white">
                          {specifications.map((spec, sIdx) => {
                            const isStandardKey = PREDEFINED_SPEC_KEYS.some(
                              (k) => k.toLowerCase() === (spec.key || '').toLowerCase()
                            );
                            const matchedStandardKey = PREDEFINED_SPEC_KEYS.find(
                              (k) => k.toLowerCase() === (spec.key || '').toLowerCase()
                            );
                            const valSuggestions = getSpecValueSuggestions(spec.key);
                            ...
                            return (
                              <tr key={sIdx} className="hover:bg-slate-50/60 transition-colors">
                                ...
                                <td className="py-2.5 px-3">
                                  <div className="space-y-1.5">
                                    <select
                                      value={isStandardKey ? matchedStandardKey : (spec.key ? '__custom__' : '')}
                                      onChange={(e) => {
                                        const val = e.target.value;
                                        if (val === '__custom__') {
                                          handleUpdateSpecification(sIdx, 'key', isStandardKey ? '' : spec.key);
                                        } else {
                                          handleUpdateSpecification(sIdx, 'key', val);
                                        }
                                      }}
                                      className="w-full px-2.5 py-1.5 border border-slate-300 rounded-lg text-xs font-semibold text-slate-800 bg-slate-50 hover:bg-white focus:bg-white focus:outline-none focus:ring-1 focus:ring-brand-500 focus:border-brand-500 cursor-pointer"
                                    >
                                      <option value="">-- Select Standard Key --</option>
                                      <optgroup label="Shop Filter Keys">
                                        <option value="Brand">Brand (HP, Dell, Lenovo...)</option>
                                        <option value="Processor">Processor (Core i3, Ryzen...)</option>
                                        <option value="RAM">RAM (8 GB, 16 GB...)</option>
                                        <option value="Storage">Storage (SSD / HDD)</option>
                                      </optgroup>
                                      <optgroup label="Other Standard Hardware Specs">
                                        <option value="Display">Display (Screen Size)</option>
                                        <option value="Graphics">Graphics (GPU)</option>
                                        <option value="Operating System">Operating System (OS)</option>
                                        <option value="Warranty">Warranty</option>
                                      </optgroup>
                                      <option value="__custom__">✏️ Custom Key Name...</option>
                                    </select>

                                    {(!isStandardKey || spec.key === '') && (
                                      <input
                                        type="text"
                                        value={spec.key}
                                        onChange={(e) => handleUpdateSpecification(sIdx, 'key', e.target.value)}
                                        placeholder="Type custom key name..."
                                        className="w-full px-2.5 py-1 border border-amber-300 bg-amber-50/40 rounded-lg text-xs font-medium text-slate-900 focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-amber-500 placeholder:text-slate-400"
                                      />
                                    )}
                                  </div>
                                </td>
                                <td className="py-2.5 px-3">
                                  <div className="relative">
                                    ...
                                    {valSuggestions.length > 0 && (
                                      <datalist id={`spec-val-list-${sIdx}`}>
                                        {valSuggestions.map((val) => (
