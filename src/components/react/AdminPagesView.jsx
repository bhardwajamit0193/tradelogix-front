...
export default function AdminPagesView() {
  ...
  const handleDeletePage = async () => {
    ...
    setDeleting(true);
    try {
      await deleteAdminPageApi(deleteConfirmPage.id);
      setPages((prev) => prev.filter((p) => p.id !== deleteConfirmPage.id));
      showToast(`Page '${deleteConfirmPage.title}' deleted.`);
      setDeleteConfirmPage(null);
    } catch (err) {
      console.error('Delete page error:', err);
      showToast(err.message || 'Could not delete page.', 'error');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      ...
      {deleteConfirmPage && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-slate-500">
                ...
                <span className="font-bold text-slate-800">"{deleteConfirmPage.title}"</span> ({getStorefrontUrl(deleteConfirmPage.slug)})?
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold shadow-md shadow-rose-600/20 transition-all flex items-center justify-center gap-2"
              >
                {deleting && <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></span>}
